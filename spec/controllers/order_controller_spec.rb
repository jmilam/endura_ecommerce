...
RSpec.describe OrderController, type: :controller do
  ...
  describe "PUT #update" do
  	describe "Saves updates to order successfully" do
	  	...
	  	it "should not update Funds Bank when approved but includes 'Other Sample'" do
	  		product = FactoryGirl.create(:other_sample_product)
	  		@order.order_items.create(item_type: "product", reference_id: product.id, quantity: 2, item_total: 0.0, note: "")
	  		put :update, params: {accepted: true, job: "approve", company_id: @customer.id, id: @order.id}
	  		expect(Customer.last.funds_bank.current_bal).to eq(3000.0)
	  	end

	  	it "should not update Funds Bank when approved" do
	  		@order.update(payment_method: "Customer PO")
	  		put :update, params: {accepted: true, job: "approve", company_id: @customer.id, id: @order.id}
	  		expect(Customer.last.funds_bank.current_bal).to eq(3000.0)
	  	end
