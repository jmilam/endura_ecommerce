class UserPortalController < ApplicationController
  def index
    @user = current_user
    #@product = Product.new
    @form_params = params[:redirected_form_params].nil? ? Hash.new : URI.decode_www_form(params[:redirected_form_params]).to_h
    ...
    @groups = Hash.new
    @products.map {|product| @groups[product.group.strip] = Array.new }.uniq
    @products.each {|product| @groups[product.group.strip].push(product)}
